...
function install_k8s_deps {
  ...
  if [[ ! -f /etc/apt/sources.list.d/kubernetes.list ]] ; then
    ...
    if [[ $(lsb_release -cs) = "trusty" ]]; then
      add_apt_key https://robco-packages.storage.googleapis.com/doc/gpg-public.key
      sudo tee -a /etc/apt/sources.list.d/kubernetes.list >/dev/null \
        <<<  "deb https://robco-packages.storage.googleapis.com/ trusty main"
    fi

    sudo apt-get update
  fi

  # If ubuntu version <= trusty (=14.04), use the backported packages from the robco-packages
