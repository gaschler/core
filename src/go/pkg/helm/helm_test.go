...
func newHelmWithDefaults(t *testing.T) *Helm {
	slamRole := roleWithNamesAndApps(
		"slam",
		"SLAM",
		roleAppWithNameAndValues("robco-freight", "app:\n  role: slam\n  sim_suffix: ''\n"))
	...
	freightApp := appWithNameAndCharts(
		"robco-freight",
		chartWithNameAndTarget("robco-freight-cloud", "CLOUD_PER_ROBOT"),
		chartWithNameAndTargetAndValues("robco-freight-robot", "ROBOT", "cloud: false"),
	...
	*params = "project=foo-project,domain=https://cloudrobotics.com"
	...
	wantRequirements := requirements{
		Dependencies: []dependency{
			{
				Name:    "robco-freight-robot",
				Version: "0.0.1",
				Alias:   "robot-wall-e-app-robco-freight",
			},
			{
				Name:    "robco-freight-robot",
				Version: "0.0.1",
				Alias:   "robot-terminator-app-robco-freight",
			},
		},
	}
	wantValues := `
robot-wall-e-app-robco-freight:
  app:
    role: nav
    sim_suffix: -sim
  cloud: false
  ...
  robot:
    ...
    role: "Nav (simulated)"
    type: Freight
robot-terminator-app-robco-freight:
  app:
    role: slam
    sim_suffix: ""
  cloud: false
	...
	*params = "project=foo-project,domain=https://cloudrobotics.com"
	...
	want := &masterChart{
		...
		values: map[string]interface{}{
			"robco-freight": map[string]interface{}{
				"cloud":   true,
				"domain":  "https://cloudrobotics.com",
				"project": "foo-project",
			},
...
func TestSetUpChart(t *testing.T) {
	...
	mc := &masterChart{
		...
		values: map[string]interface{}{
			"robot-wall-e-app-robco-freight": map[string]interface{}{
...
`
	...
	expValues := `robot-wall-e-app-robco-freight:
  app:
    role: nav
    sim_suffix: -sim
  cloud: true
